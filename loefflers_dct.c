...
void dct_round2(register int16_t X[240][320]) {
	...
	for (i = 0; i < 240; i+=8) {
		for (j = 0; j < 320; j++) {
			...
			// int32_t r0, r1, r2, r3, r4, r5;
			// r0 = ((X[i+1][j])<<16)|(X[i+0][j]);
			// r1 = ((X[i+3][j] & 0xffff)<<16)|(X[i+2][j] & 0xffff);
			// r2 = ((X[i+5][j] & 0xffff)<<16)|(X[i+4][j] & 0xffff);
			// r3 = ((X[i+7][j] & 0xffff)<<16)|(X[i+6][j] & 0xffff);
			...
			// X[i+0][j] = round_5(r0&0xffff);
			// X[i+1][j] = round_5(r5);
			// X[i+2][j] = round_5(r1&0xffff);
			// X[i+3][j] = round_5(r4);
